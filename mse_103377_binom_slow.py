...
def find_n_k(x):
    ...
    while True:
        ...
        if (2 * k + 1) * x <= 4**k:
            break
        kfx = math.factorial(k) * x
        # Binary search to find smallest n such that n^k >= k!X
        n = 1
        while n ** k < kfx:
            n *= 2
        # Invariant: lo**k < kfx <= hi**k
        lo = 1
        hi = n
        while hi - lo > 1:
            mid = lo + (hi - lo) // 2
        ...
        assert hi ** k >= kfx
        assert (hi - 1) ** k < kfx
        for n in range(hi, hi + k + 1):
            if binom(n, k) != x:
                continue
            if k > n - k:
                # We'd have already found this (n, k) for a smaller k
                continue
            yield (n, k)
            if k < n - k:
                yield (n, n - k)
